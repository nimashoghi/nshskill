...
`nshskill` is a small library that eliminates boilerplate for shipping Claude Code skills from Python packages. It handles SKILL.md frontmatter parsing, file copying to `.claude/skills/<name>/`, and argparse CLI wiring.
...
├── _cli.py        # nshskill's own CLI entry point (dogfoods create_skill_cli)
└── _skill/
    └── SKILL.md   # init-skill: Claude Code skill for bootstrapping nshskill integrations
```
...
- **`_cli.py`** — nshskill's own CLI entry point, exposed as `nshskill` command. Dogfoods `create_skill_cli()`.
- **`_skill/SKILL.md`** — The `init-skill` skill. When installed and invoked via `/init-skill`, instructs Claude to analyze a codebase and set up a full nshskill integration.

## Claude Code Skill

nshskill ships an `init-skill` skill for bootstrapping nshskill integrations:
